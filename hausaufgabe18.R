...
# 2014-06-18
# Dieses Werk dient nur zu Prüfugszwecken.
...
pyreg <- read.table("Franzis/pyreg.tab",header=TRUE)
?pyreg
??pyreg
...
# CODE_HIER
model5 <- lm(y ~ x2 + x1, data=pyreg)
model5.summary <- summary(model5)
print(model5.summary)


# Wie gut passt das lineare Modell zu den Daten? Schauen Sie sich die R^2 und 
# F-Werte an sowie auch die t-Werte für die einzelnen Prediktoren. Mit R^2 von 0.96 scheint das Modell sehr gut zu den Daten zu passen
# Glauben Sie, 
