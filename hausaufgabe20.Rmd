...
`r citet("10.1016/j.wocn.2012.08.006")` 
...
Der Standard fehler ist jetzt Intercept:13.8; genderM:17.6; und attituepol:5.5, vorher war er beim Intercept ganz wenig geringer und bei genderM ganz wenig größer. Die Unterschiede sind aber zu vernachlässigen. Neu hier ist attitudepol mit Standardfehler 5.55

Jetzt berechen wir das Modell mit der Interaktion zwischen `gender` und `attitude`. 

```{r}
#model.attitude.int <- CODE HIER:
model.attitude <- lmer(frequency ~ gender + attitude + (1|subject) + (1|scenario), data=stimmen, REML=FALSE)
